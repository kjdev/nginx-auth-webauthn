...
./configure \
  ...
  --add-dynamic-module=..
make modules
cp objs/ngx_http_auth_webauthn_module.so /usr/lib/nginx/modules/
EOS

## build cli
WORKDIR /build/tools/admin
RUN sh -ex <<'EOS'
make
EOS
...
apk add \
  ...
  jansson \
  libcbor
## load module: ngx_http_auth_webauthn_module.so
sed -i '/events {/i load_module "/usr/lib/nginx/modules/ngx_http_auth_webauthn_module.so";' /etc/nginx/nginx.conf
EOS

COPY --from=builder /usr/lib/nginx/modules/ngx_http_auth_webauthn_module.so /usr/lib/nginx/modules/ngx_http_auth_webauthn_module.so
COPY --from=builder /build/tools/admin/auth-webauthn-admin /usr/local/bin/auth-webauthn-admin
